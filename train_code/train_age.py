...
class SaveEvalResultsCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        eval_file = "training_eval/eval_result_age.json"
...
def load_data_char_based(task1_path, task2_path):
    ...
    with open(task1_path, encoding='utf-8') as f:
        for idx, line in enumerate(f, 1):
            ...
            parts = line.split('\t', 1)
            if len(parts) != 2:
                print(f"[Line {idx}] Skipping malformed line: {line}")
                continue
            key, text = parts
            texts[key] = text
    annotations = {}
...
train_data = load_data_char_based("train_data/task1_age.txt", "train_data/task2_age.txt")
...
args = TrainingArguments(
    output_dir="model/ner_model_age",
...
model.save_pretrained("model/ner_model_age")
tokenizer.save_pretrained("model/ner_model_age")
print("模型訓練與儲存完成！")