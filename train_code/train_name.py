...
ALL_SHI_TYPES = [
    "PATIENT","FAMILYNAME","PERSONALNAME"
]
...
class SaveEvalResultsCallback(TrainerCallback):
    def on_evaluate(self, args, state, control, metrics, **kwargs):
        ...
        eval_file = "training_eval/eval_result_name.json"
...
def load_data_char_based(task1_path, task2_path):
    ...
    with open(task1_path, encoding='utf-8') as f:
        for i, line in enumerate(f, 1):
            ...
            parts = line.split('\t', 1)
            if len(parts) != 2:
                print(f" 第 {i} 行格式錯誤（應為 sid<TAB>text），已跳過：{line}")
                continue
            sid, text = parts
            texts[sid] = text
    annotations = {}
...
train_data = load_data_char_based("train_data/task1_name.txt", "train_data/task2_name.txt")
...
args = TrainingArguments(
    output_dir="model/ner_model_name",
...
model.save_pretrained("model/ner_model_name")
tokenizer.save_pretrained("model/ner_model_name")
